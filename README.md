...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create label-printer-85cd
  fly apps create label-printer-85cd-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app label-printer-85cd
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app label-printer-85cd-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'label-printer-85cd-staging' does not match app name in config file 'label-printer-85cd'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name label-printer-85cd-db
  fly postgres attach --app label-printer-85cd label-printer-85cd-db

  fly postgres create --name label-printer-85cd-staging-db
  fly postgres attach --app label-printer-85cd-staging label-printer-85cd-staging-db
  ```
